...
\hypertarget{md_docs_tutorials_topical_multitasking_autotoc_md66}{}\doxysection{On the Abuse of Tasks}\label{md_docs_tutorials_topical_multitasking_autotoc_md66}
...
\end{DoxyItemize}\end{quote}
\hypertarget{md_docs_tutorials_topical_multitasking_autotoc_md67}{}\doxysubsection{Task Management}\label{md_docs_tutorials_topical_multitasking_autotoc_md67}
...
The last parameter is the task name. The task name allows you to give a task a human-\/friendly name for the task. It is primarily for debugging purposes and allows you (the human) to easily identify tasks if performing advanced task management. Task names may be up to 32 characters long, and you may pass N\+U\+LL or an empty string into the function. In A\+P\+I2, \href{../../../cortex/api/index.html\#taskCreate}{\texttt{ task\+Create}} will automatically make the task name an empty string.\hypertarget{md_docs_tutorials_topical_multitasking_autotoc_md68}{}\doxysubsection{Synchronization}\label{md_docs_tutorials_topical_multitasking_autotoc_md68}
...
Mutexes do not magically prevent concurrent writing, but provide the ability for tasks to create \char`\"{}contracts\char`\"{} with each other. You can write your code such that a variable is never written to unless the task owns a mutex designated for that variable.\hypertarget{md_docs_tutorials_topical_multitasking_autotoc_md70}{}\doxysection{Notifications}\label{md_docs_tutorials_topical_multitasking_autotoc_md70}
