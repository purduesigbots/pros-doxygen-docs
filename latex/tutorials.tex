...
practical applications, or teach students to use P\+R\+OS in a classroom environment, the {\bfseries{Walkthrough Tutorials}} are a great resource to go step by step through the process of writing a P\+R\+OS program.
...
\hypertarget{debugging_autotoc_md18}{}\doxyparagraph{Further debug info with $<$tt$>$errno$<$/tt$>$}\label{debugging_autotoc_md18}
...
P\+R\+OS projects are internally composed of three parts\+: the {\bfseries{P\+R\+OS library}} (found in {\ttfamily /firmware}), the {\bfseries{header}} files (found in {\ttfamily /include}), and {\bfseries{user code}} (found in {\ttfamily /src}).\hypertarget{project-structure_autotoc_md20}{}\doxyparagraph{firmware}\label{project-structure_autotoc_md20}
The {\bfseries{P\+R\+OS library}} is a single file containing the core P\+R\+OS routines. This file does not need to be changed. If there appears to be an issue with a core P\+R\+OS function, please file an issue on \href{https://github.com/purduesigbots/pros/issues}{\texttt{ Git\+Hub}}.\hypertarget{project-structure_autotoc_md21}{}\doxyparagraph{include}\label{project-structure_autotoc_md21}
...
\hypertarget{troubleshooting_autotoc_md23}{}\doxyparagraph{Why do I get a \char`\"{}\+Could not open port\char`\"{} error when flashing?}\label{troubleshooting_autotoc_md23}
...
That means that some other resource is using the port. If you also have the V5 Utility open, that would cause this error to occur. Close the V5 Utility if you want to flash.\hypertarget{troubleshooting_autotoc_md24}{}\doxyparagraph{Compile-\/\+Time Issues}\label{troubleshooting_autotoc_md24}
\begin{quote}
...
\end{DoxyItemize}\end{quote}
\hypertarget{troubleshooting_autotoc_md25}{}\doxyparagraph{Run-\/\+Time Issues}\label{troubleshooting_autotoc_md25}
\begin{quote}
...
\end{DoxyNote}
\hypertarget{adi_autotoc_md29}{}\doxyparagraph{Analog Sensors}\label{adi_autotoc_md29}
...
Thus an example of use on a lift would look like\+:\hypertarget{adi_autotoc_md32}{}\doxysubparagraph{Line Tracker}\label{adi_autotoc_md32}
...
An example of Line Tracker use\+:\hypertarget{adi_autotoc_md33}{}\doxysubparagraph{Accelerometer}\label{adi_autotoc_md33}
...
Example accelerometer use\+:\hypertarget{adi_autotoc_md34}{}\doxyparagraph{Digital Sensors}\label{adi_autotoc_md34}
\hypertarget{adi_autotoc_md35}{}\doxysubparagraph{Initialization}\label{adi_autotoc_md35}
As with all A\+DI sensors, the first step to using the sensor is to set the configuration for its A\+DI port.

From there, using a digital sensor is fairly straightforward. Digital Sensors always return a true or false (boolean) value.\hypertarget{adi_autotoc_md36}{}\doxysubparagraph{Quad Encoder}\label{adi_autotoc_md36}
...
\end{DoxyNote}
And then used in the following manner\+:\hypertarget{adi_autotoc_md37}{}\doxysubparagraph{Ultrasonic}\label{adi_autotoc_md37}
...
\end{DoxyNote}
And then used in the following manner\+:\hypertarget{adi_autotoc_md38}{}\doxysubparagraph{Pneumatics}\label{adi_autotoc_md38}
...
\end{DoxyNote}
The analog data is a value in the range of \mbox{[}-\/127,127\mbox{]}, and digital data is either 1 or 0 (pressed or unpressed, respectively).\hypertarget{controller_autotoc_md39}{}\doxyparagraph{Analog Data}\label{controller_autotoc_md39}
Retrieve analog values from the V5 Controller in the following manner\+:

The controller returns a range of \mbox{[}-\/127,127\mbox{]} for analog data, which is why the \href{../../api/c/motors.html\#motor-move}{\texttt{ motor\+\_\+move}} function is appropriate for easy use with the controllers.\hypertarget{controller_autotoc_md40}{}\doxyparagraph{Digital Data}\label{controller_autotoc_md40}
...
\end{DoxyCode}
\hypertarget{filesystem_autotoc_md41}{}\doxyparagraph{Remarks}\label{filesystem_autotoc_md41}
The micro\+SD card must be fat32 in order to work.\hypertarget{filesystem_autotoc_md42}{}\doxysubparagraph{Serial}\label{filesystem_autotoc_md42}
...
\end{DoxyNote}
\hypertarget{llemu_autotoc_md43}{}\doxyparagraph{Initialization}\label{llemu_autotoc_md43}
...
Initialization is done as such\+:\hypertarget{llemu_autotoc_md44}{}\doxyparagraph{Writing to the L\+L\+E\+MU}\label{llemu_autotoc_md44}
Writing to the L\+L\+E\+MU is nearly identical to writing to the L\+CD with \href{../../cortex/tutorials/lcd.html}{\texttt{ P\+R\+OS 2}}. Most writing should be done with the print function, which is analogous to \href{http://www.cplusplus.com/reference/cstdio/printf/}{\texttt{ printf}}.\hypertarget{llemu_autotoc_md45}{}\doxyparagraph{Using the Buttons}\label{llemu_autotoc_md45}
...
\end{DoxyNote}
\hypertarget{motors_autotoc_md46}{}\doxyparagraph{Initialization}\label{motors_autotoc_md46}
...
\end{quote}
\end{quote}
\hypertarget{motors_autotoc_md47}{}\doxyparagraph{Simple Usage}\label{motors_autotoc_md47}
The easiest way to interact with the motors is through the \href{../../api/c/motors.html\#motor-move}{\texttt{ motor\+\_\+move}} function. This is analogous to the \href{../../../cortex/api/index.html\#motorSet}{\texttt{ motor\+Set}} function from P\+R\+OS 2.\hypertarget{motors_autotoc_md48}{}\doxyparagraph{Autonomous Movement}\label{motors_autotoc_md48}
The V5 Motors can move in a number of different ways that are better suited towards autonomous movement than the simple {\ttfamily \mbox{\hyperlink{motors_8h_a7da9bf1e229e50bfeeaecf026a6d0d08}{motor\+\_\+move()}}} example shown above.\hypertarget{motors_autotoc_md49}{}\doxysubparagraph{Profile Movements}\label{motors_autotoc_md49}
