...
The P\+R\+OS tutorials are split into three sections\+:\hypertarget{tutorials_autotoc_md31}{}\doxysection{General Tutorials}\label{tutorials_autotoc_md31}
...
\begin{DoxyItemize}
\item \mbox{\hyperlink{faq}{Coding F\+AQ}}
...
\end{DoxyItemize}\hypertarget{tutorials_autotoc_md32}{}\doxysection{Walkthrough Tutorials}\label{tutorials_autotoc_md32}
...
\item \mbox{\hyperlink{uploading}{Uploading}}
\end{DoxyItemize}\hypertarget{tutorials_autotoc_md33}{}\doxysection{Topical Tutorials}\label{tutorials_autotoc_md33}
...
\item \mbox{\hyperlink{project-structure}{Project Structure}}
\item \mbox{\hyperlink{troubleshooting}{P\+R\+OS Troubleshooting and Common Problems}} 
\end{DoxyItemize}\hypertarget{faq}{}\doxysubsection{Coding F\+AQ}\label{faq}
\hypertarget{faq_autotoc_md19}{}\doxyparagraph{Coding F\+AQ}\label{faq_autotoc_md19}
...
\end{DoxyItemize}\hypertarget{faq_autotoc_md20}{}\doxyparagraph{Compile-\/\+Time Issues}\label{faq_autotoc_md20}
...
\item {\ttfamily format ... expects argument of type ..., but argument has type ...}\+: The value provided to a function like {\ttfamily printf $<$\href{http://www.cplusplus.com/reference/cstdio/printf/}{\texttt{ http\+://www.\+cplusplus.\+com/reference/cstdio/printf/}}$>$}\+\_\+ or {\ttfamily lcd\+\_\+print $<$../../api/c/llemu.html\#lcd-\/print$>$}\+\_\+ does not match the expected type inferred from the format string. Some instances of this warning can be safely ignored, but crashes can occur if types {\ttfamily double} or {\ttfamily long long} are mixed with other types.
\item {\ttfamily assignment makes pointer from integer without a cast}\+: Typically caused when a C pointer has the wrong number of asterisks to {\ttfamily dereference $<$\href{http://stackoverflow.com/a/4955297/3681958}{\texttt{ http\+://stackoverflow.\+com/a/4955297/3681958}}$>$}\+\_\+ it, or when assigning a constant to {\ttfamily pointer} (instead of {\ttfamily $\ast$pointer}).
\end{DoxyItemize}\hypertarget{faq_autotoc_md21}{}\doxyparagraph{Run-\/\+Time Issues}\label{faq_autotoc_md21}

\begin{DoxyItemize}
\item {\bfseries{Some tasks are running, others are not\+:}} A task is not waiting using {\ttfamily delay $<$../../api/c/rtos.html\#delay$>$}\+\_\+ or {\ttfamily task\+\_\+delay\+\_\+until $<$../../api/c/rtos.html\#task-\/delay-\/until$>$}\+\_\+. Due to the fact that P\+R\+OS utilizes a priority based non-\/preemptive scheduler, tasks of higher or equal priority to the blocking task will still run while lower priority tasks will not. This scenario is also known as {\ttfamily starvation $<$\href{https://en.wikipedia.org/wiki/Starvation_(computer_science)}{\texttt{ https\+://en.\+wikipedia.\+org/wiki/\+Starvation\+\_\+(computer\+\_\+science)}}$>$}\+\_\+. See {\ttfamily Tasks/\+Multithreading $<$/tutorials/topical/multitasking$>$}\+\_\+ for more information.
\item {\bfseries{V\+EX L\+CD updates very slowly or is \char`\"{}frozen\char`\"{}\+:}} A task is not waiting using {\ttfamily delay $<$../../api/c/rtos.html\#delay$>$}\+\_\+ or {\ttfamily task\+\_\+delay\+\_\+until $<$../../api/c/rtos.html\#task-\/delay-\/until$>$}\+\_\+. From the kernel\textquotesingle{}s perspective, updating the L\+CD is usually less important than how well the robot is running, so P\+R\+OS prioritizes user tasks over the L\+CD.

The L\+CD is only updated if all other tasks are waiting.
\item {\bfseries{Neither autonomous nor driver control code starts\+:}} The {\ttfamily \mbox{\hyperlink{main_8h_a9efe22aaead3a5e936b5df459de02eba}{initialize()}}} function may still be running. Some tasks such as {\ttfamily analog\+\_\+calibrate $<$../../api/c/adi.html\#analog-\/calibrate$>$}\+\_\+ take time.

If the {\ttfamily \mbox{\hyperlink{main_8h_a9efe22aaead3a5e936b5df459de02eba}{initialize()}}} function implements some type of loop or autonomous selection routine, verify that it actually has a means of ending.
\item {\bfseries{Code restarts unexpectedly\+:}} A run-\/time error has caused the program to crash. {\ttfamily Debugging $<$./debugging$>$}\+\_\+ may reveal the cause of the error. Examine any newly added code for possible logical errors. Some common error messages include\+:
\begin{DoxyItemize}
...
\item {\ttfamily printf $<$printf\+\_\+$>$}\+\_\+ {\bfseries{doesn\textquotesingle{}t work}}\+: {\ttfamily printf $<$\href{http://www.cplusplus.com/reference/cstdio/printf/}{\texttt{ http\+://www.\+cplusplus.\+com/reference/cstdio/printf/}}$>$}\+\_\+ prints information over a serial connection ({\ttfamily Debugging $<$../general/debugging$>$}\+\_\+), not to the V\+EX L\+CD. To print to the L\+CD, use {\ttfamily lcd\+\_\+print $<$../../api/c/llemu.html\#lcd-\/print$>$}\+\_\+ instead.
\end{DoxyItemize}

.. \+\_\+printf\+: \href{http://www.cplusplus.com/reference/cstdio/printf/}{\texttt{ http\+://www.\+cplusplus.\+com/reference/cstdio/printf/}} \hypertarget{combining-c-cpp}{}\doxysubsection{Combining C and C++}\label{combining-c-cpp}
The P\+R\+OS C and C++ A\+P\+Is cover the exact same feature set, which means that there is no significant advantage to using functions from one A\+PI in another language. However, if you do find yourself wanting to use C A\+PI functions in C++, these functions are accessible through the namespace {\ttfamily \mbox{\hyperlink{namespacepros_1_1c}{pros\+::c}}}.
...
\end{DoxyImageNoCaption}
  \hypertarget{debugging_autotoc_md23}{}\doxyparagraph{Further debug info with $<$tt$>$errno$<$/tt$>$}\label{debugging_autotoc_md23}
...
P\+R\+OS projects are internally composed of three parts\+: the {\bfseries{P\+R\+OS library}} (found in {\ttfamily /firmware}), the {\bfseries{header}} files (found in {\ttfamily /include}), and {\bfseries{user code}} (found in {\ttfamily /src}).\hypertarget{project-structure_autotoc_md25}{}\doxyparagraph{firmware}\label{project-structure_autotoc_md25}
The {\bfseries{P\+R\+OS library}} is a single file containing the core P\+R\+OS routines. This file does not need to be changed. If there appears to be an issue with a core P\+R\+OS function, please file an issue on \href{https://github.com/purduesigbots/pros/issues}{\texttt{ Git\+Hub}}.\hypertarget{project-structure_autotoc_md26}{}\doxyparagraph{include}\label{project-structure_autotoc_md26}
...
\hypertarget{troubleshooting_autotoc_md28}{}\doxyparagraph{Why do I get a \char`\"{}\+Could not open port\char`\"{} error when flashing?}\label{troubleshooting_autotoc_md28}
...
That means that some other resource is using the port. If you also have the V5 Utility open, that would cause this error to occur. Close the V5 Utility if you want to flash.\hypertarget{troubleshooting_autotoc_md29}{}\doxyparagraph{Compile-\/\+Time Issues}\label{troubleshooting_autotoc_md29}
\begin{quote}
...
\end{DoxyItemize}\end{quote}
\hypertarget{troubleshooting_autotoc_md30}{}\doxyparagraph{Run-\/\+Time Issues}\label{troubleshooting_autotoc_md30}
\begin{quote}
...
\end{DoxyItemize}\hypertarget{clawbot_autotoc_md72}{}\doxyparagraph{The Clawbot}\label{clawbot_autotoc_md72}
...
A Left Bumper E ~\newline
 B Right Bumper F ~\newline
 C G ~\newline
 D H Arm Limit\hypertarget{clawbot_autotoc_md75}{}\doxyparagraph{Creating the Project}\label{clawbot_autotoc_md75}
...
Pick a directory to create the new project in and click Create. The P\+R\+OS C\+LI will now copy the latest kernel template into the specified directory and Atom will open it.\hypertarget{clawbot_autotoc_md76}{}\doxyparagraph{P\+R\+O\+S Project Structure}\label{clawbot_autotoc_md76}
...
\end{DoxyNote}
\hypertarget{clawbot_autotoc_md77}{}\doxyparagraph{Drive Control}\label{clawbot_autotoc_md77}
...
These 2 commands can be simplified to {\ttfamily \`{}prosv5 mu}.\hypertarget{clawbot_autotoc_md78}{}\doxyparagraph{Arcade Control}\label{clawbot_autotoc_md78}
...
As with the tank drive code, this can be uploaded with the {\ttfamily prosv5 mu} command.\hypertarget{clawbot_autotoc_md79}{}\doxyparagraph{Arm Control}\label{clawbot_autotoc_md79}
...
To actuate the lift, we will check if the upmost right trigger is pressed or if the bottommost right trigger is pressed on the controller, and move the lift in that direction if so.\hypertarget{clawbot_autotoc_md80}{}\doxyparagraph{Claw Control}\label{clawbot_autotoc_md80}
We will control the claw in the same manner as the lift, by toggling its movement with a controller button.\hypertarget{clawbot_autotoc_md81}{}\doxyparagraph{Reading the Switches}\label{clawbot_autotoc_md81}
...
We will use a similar technique for reading the limit switch. If the limit switch is pressed, then we will prevent the lift from being driven down further.\hypertarget{clawbot_autotoc_md82}{}\doxyparagraph{Simple Autonomous}\label{clawbot_autotoc_md82}
...
\end{DoxyWarning}
This will compile your P\+R\+OS project as C code, and will give you access to the C A\+PI.

If you\textquotesingle{}re interested in combining C and C++, you should read through the \mbox{\hyperlink{combining-c-cpp}{Combining C and C++}}. Please note that it is typically a complicated matter to combine the two, and rarely a good idea. \hypertarget{uploading}{}\doxysubsection{Uploading}\label{uploading}
\hypertarget{uploading_autotoc_md83}{}\doxyparagraph{Uploading}\label{uploading_autotoc_md83}
\hypertarget{uploading_autotoc_md84}{}\doxyparagraph{Uploading from within the Editor}\label{uploading_autotoc_md84}

\begin{DoxyItemize}
\item Make and Upload your project\+:

By clicking the P\+R\+OS Quick Action button the editor will make and upload your project.

.. image\+:\+: /images/atom/quick-\/action.png
\end{DoxyItemize}\hypertarget{uploading_autotoc_md85}{}\doxyparagraph{Uploading through the P\+R\+O\+S C\+LI}\label{uploading_autotoc_md85}

\begin{DoxyItemize}
\item Running {\ttfamily pros upload} on the command line will upload your project to the V5 brain.
\end{DoxyItemize}\hypertarget{uploading_autotoc_md86}{}\doxyparagraph{Wireless Upload}\label{uploading_autotoc_md86}
To see more details on how to wireless upload see \mbox{\hyperlink{wireless-upload}{Wireless Upload and Hot/\+Cold Linking}} \hypertarget{topical-tutorials}{}\doxysection{Topical Tutorials}\label{topical-tutorials}
If you want more information about using a particular part of the \mbox{\hyperlink{api}{A\+PI Home}}, the {\bfseries{Topical Tutorials}} offer longer examples and more detail about using the P\+R\+OS A\+PI.
...
\end{DoxyNote}
\hypertarget{adi_autotoc_md34}{}\doxyparagraph{Analog Sensors}\label{adi_autotoc_md34}
...
Thus an example of use on a lift would look like\+:\hypertarget{adi_autotoc_md37}{}\doxysubparagraph{Line Tracker}\label{adi_autotoc_md37}
...
An example of Line Tracker use\+:\hypertarget{adi_autotoc_md38}{}\doxysubparagraph{Accelerometer}\label{adi_autotoc_md38}
...
Example accelerometer use\+:\hypertarget{adi_autotoc_md39}{}\doxyparagraph{Digital Sensors}\label{adi_autotoc_md39}
\hypertarget{adi_autotoc_md40}{}\doxysubparagraph{Initialization}\label{adi_autotoc_md40}
As with all A\+DI sensors, the first step to using the sensor is to set the configuration for its A\+DI port.

From there, using a digital sensor is fairly straightforward. Digital Sensors always return a true or false (boolean) value.\hypertarget{adi_autotoc_md41}{}\doxysubparagraph{Quad Encoder}\label{adi_autotoc_md41}
...
And then used in the following manner\+:\hypertarget{adi_autotoc_md42}{}\doxysubparagraph{Ultrasonic}\label{adi_autotoc_md42}
...
And then used in the following manner\+:\hypertarget{adi_autotoc_md43}{}\doxysubparagraph{Pneumatics}\label{adi_autotoc_md43}
...
\end{DoxyNote}
The analog data is a value in the range of \mbox{[}-\/127,127\mbox{]}, and digital data is either 1 or 0 (pressed or unpressed, respectively).\hypertarget{controller_autotoc_md44}{}\doxyparagraph{Analog Data}\label{controller_autotoc_md44}
Retrieve analog values from the V5 Controller in the following manner\+:

The controller returns a range of \mbox{[}-\/127,127\mbox{]} for analog data, which is why the \href{../../api/c/motors.html\#motor-move}{\texttt{ motor\+\_\+move}} function is appropriate for easy use with the controllers.\hypertarget{controller_autotoc_md45}{}\doxyparagraph{Digital Data}\label{controller_autotoc_md45}
...
\end{DoxyCode}
\hypertarget{filesystem_autotoc_md46}{}\doxyparagraph{Remarks}\label{filesystem_autotoc_md46}
The micro\+SD card must be fat32 in order to work.\hypertarget{filesystem_autotoc_md47}{}\doxysubparagraph{Serial}\label{filesystem_autotoc_md47}
...
\end{DoxyNote}
\hypertarget{llemu_autotoc_md48}{}\doxyparagraph{Initialization}\label{llemu_autotoc_md48}
...
Initialization is done as such\+:\hypertarget{llemu_autotoc_md49}{}\doxyparagraph{Writing to the L\+L\+E\+MU}\label{llemu_autotoc_md49}
Writing to the L\+L\+E\+MU is nearly identical to writing to the L\+CD with \href{../../cortex/tutorials/lcd.html}{\texttt{ P\+R\+OS 2}}. Most writing should be done with the print function, which is analogous to \href{http://www.cplusplus.com/reference/cstdio/printf/}{\texttt{ printf}}.\hypertarget{llemu_autotoc_md50}{}\doxyparagraph{Using the Buttons}\label{llemu_autotoc_md50}
...
\end{DoxyNote}
\hypertarget{motors_autotoc_md51}{}\doxyparagraph{Initialization}\label{motors_autotoc_md51}
...
\DoxyCodeLine{  \mbox{\hyperlink{group__cpp-motors_classpros_1_1v5_1_1Motor}{pros::Motor}} drive\_left (MOTOR\_PORT);}
\DoxyCodeLine{  \textcolor{comment}{// drive\_left will have the same configuration as drive\_left\_initializer}}
\DoxyCodeLine{\}}
\end{DoxyCode}
\hypertarget{motors_autotoc_md52}{}\doxyparagraph{Simple Usage}\label{motors_autotoc_md52}
The easiest way to interact with the motors is through the \href{../../api/c/motors.html\#motor-move}{\texttt{ motor\+\_\+move}} function.\hypertarget{motors_autotoc_md53}{}\doxyparagraph{Autonomous Movement}\label{motors_autotoc_md53}
The V5 Motors can move in a number of different ways that are better suited towards autonomous movement than the simple {\ttfamily \mbox{\hyperlink{group__c-motors_gac14bccd595a812fff8c30d2d050844a9}{motor\+\_\+move()}}} example shown above.\hypertarget{motors_autotoc_md54}{}\doxysubparagraph{Profile Movements}\label{motors_autotoc_md54}
...
\end{DoxyItemize}\end{quote}
\hypertarget{multitasking_autotoc_md56}{}\doxyparagraph{On the Abuse of Tasks}\label{multitasking_autotoc_md56}
...
\end{DoxyItemize}\end{quote}
\hypertarget{multitasking_autotoc_md57}{}\doxysubparagraph{Task Management}\label{multitasking_autotoc_md57}
...
The last parameter is the task name. The task name allows you to give a task a human-\/friendly name for the task. It is primarily for debugging purposes and allows you (the human) to easily identify tasks if performing advanced task management. Task names may be up to 32 characters long, and you may pass N\+U\+LL or an empty string into the function. In A\+P\+I2, \href{../../../cortex/api/index.html\#taskCreate}{\texttt{ task\+Create}} will automatically make the task name an empty string.\hypertarget{multitasking_autotoc_md58}{}\doxysubparagraph{Synchronization}\label{multitasking_autotoc_md58}
...
Mutexes do not magically prevent concurrent writing, but provide the ability for tasks to create \char`\"{}contracts\char`\"{} with each other. You can write your code such that a variable is never written to unless the task owns a mutex designated for that variable.\hypertarget{multitasking_autotoc_md60}{}\doxyparagraph{Notifications}\label{multitasking_autotoc_md60}
...
\end{DoxyNote}
\hypertarget{vision_autotoc_md61}{}\doxyparagraph{Setting Signatures}\label{vision_autotoc_md61}
The first step to using the vision sensor is setting color signatures for the sensor to recognize as objects. This is done through the V5 Vision Utility program.\hypertarget{vision_autotoc_md62}{}\doxyparagraph{Retrieving Objects}\label{vision_autotoc_md62}
The primary function of the vision sensor is returning objects, or blobs of color detected by the sensor. The characteristics of an object are defined in \href{../../api/c/vision.html\#vision_object_s_t}{\texttt{ vision\+\_\+object\+\_\+s\+\_\+t}}.\hypertarget{vision_autotoc_md63}{}\doxysubparagraph{By Size}\label{vision_autotoc_md63}
The simplest way to interact with the vision sensor is to get an object by its size. 0 is the largest object detected by the sensor.\hypertarget{vision_autotoc_md64}{}\doxysubparagraph{By Signature}\label{vision_autotoc_md64}
If you have multiple signatures saved to the vision signature, you will most likely want to only look for objects of a particular signature. The {\ttfamily get\+\_\+by\+\_\+sig()} function implements this functionality.\hypertarget{vision_autotoc_md65}{}\doxyparagraph{Changing the Object Coordinates}\label{vision_autotoc_md65}
Each returned object from the vision sensor comes with a set of coordinates telling where the object was found in the vision sensor\textquotesingle{}s field of view. The default behavior is to return the coordinates as a function of distance from the top left corner of the field of view -\/ so positive y is downward and positive x is right. With the P\+R\+OS A\+PI, you can change this behavior so that the center of the Field Of View is the (0,0) point for object coordinates. Positive y is still downward and positive x is still right, but negative y is upward of center and negative x is left of center in this configuration.\hypertarget{vision_autotoc_md66}{}\doxyparagraph{Exposure Setting}\label{vision_autotoc_md66}
...
\end{DoxyCode}
\hypertarget{wireless-upload_autotoc_md68}{}\doxyparagraph{Hot/\+Cold Linking Troubleshooting}\label{wireless-upload_autotoc_md68}
