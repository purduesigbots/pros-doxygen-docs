...
\end{DoxyItemize}\hypertarget{md_docs_tutorials_walkthrough_clawbot_autotoc_md86}{}\doxysection{The Clawbot}\label{md_docs_tutorials_walkthrough_clawbot_autotoc_md86}
...
A Left Bumper E ~\newline
 B Right Bumper F ~\newline
 C G ~\newline
 D H Arm Limit\hypertarget{md_docs_tutorials_walkthrough_clawbot_autotoc_md89}{}\doxysection{Creating the Project}\label{md_docs_tutorials_walkthrough_clawbot_autotoc_md89}
...
Pick a directory to create the new project in and click Create. The P\+R\+OS C\+LI will now copy the latest kernel template into the specified directory and Atom will open it.\hypertarget{md_docs_tutorials_walkthrough_clawbot_autotoc_md90}{}\doxysection{P\+R\+O\+S Project Structure}\label{md_docs_tutorials_walkthrough_clawbot_autotoc_md90}
...
\hypertarget{md_docs_tutorials_walkthrough_clawbot_autotoc_md91}{}\doxysection{Drive Control}\label{md_docs_tutorials_walkthrough_clawbot_autotoc_md91}
...
These 2 commands can be simplified to {\ttfamily \`{}prosv5 mu}.\hypertarget{md_docs_tutorials_walkthrough_clawbot_autotoc_md92}{}\doxysection{Arcade Control}\label{md_docs_tutorials_walkthrough_clawbot_autotoc_md92}
...
As with the tank drive code, this can be uploaded with the {\ttfamily prosv5 mu} command.\hypertarget{md_docs_tutorials_walkthrough_clawbot_autotoc_md93}{}\doxysection{Arm Control}\label{md_docs_tutorials_walkthrough_clawbot_autotoc_md93}
...
To actuate the lift, we will check if the upmost right trigger is pressed or if the bottommost right trigger is pressed on the controller, and move the lift in that direction if so.\hypertarget{md_docs_tutorials_walkthrough_clawbot_autotoc_md94}{}\doxysection{Claw Control}\label{md_docs_tutorials_walkthrough_clawbot_autotoc_md94}
We will control the claw in the same manner as the lift, by toggling its movement with a controller button.\hypertarget{md_docs_tutorials_walkthrough_clawbot_autotoc_md95}{}\doxysection{Reading the Switches}\label{md_docs_tutorials_walkthrough_clawbot_autotoc_md95}
...
We will use a similar technique for reading the limit switch. If the limit switch is pressed, then we will prevent the lift from being driven down further.\hypertarget{md_docs_tutorials_walkthrough_clawbot_autotoc_md96}{}\doxysection{Simple Autonomous}\label{md_docs_tutorials_walkthrough_clawbot_autotoc_md96}
